...
class ArrowGame:
    def main(self):
        ...
        fullname = os.path.join('images', 'shot.gif')
        image = pygame.image.load(fullname).convert()

        imageplayer = pygame.Surface((40, 40))
        ...
        while not done:
            ...
            if self.enter(x_a, y_a, x_t, y_t, 20, 40, 40, 23):
                count += 1
                strike = False
                x_a, y_a = 1000, 1000
            ...
            screen.blit(imageplayer, (x_p, y_p))
            screen.blit(tar, (x_t, y_t))
            screen.blit(image, (x_a, y_a))
            window.blit(screen, (0, 0))
            pygame.display.update()

        pygame.quit()

    def enter(self, x1, y1, x2, y2, a1, b1, a2, b2):   #Проверка попадания стрелы в цель
